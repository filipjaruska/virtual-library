...
export function Footer({ data }: Readonly<FooterProps>) {
    if (!data) {
        console.error("Footer data is undefined");
        return (
            <div className="bg-secondary text-secondary-foreground py-8">
                <div className="container mx-auto px-4 md:px-6 flex justify-center">
                    <p className="text-sm text-muted-foreground">© {new Date().getFullYear()} Virtual Library</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="bg-secondary text-secondary-foreground py-8">
            <div className="container mx-auto px-4 md:px-6 flex flex-col md:flex-row items-center justify-between">
                <div>{logoText?.text || "Virtual Library"}</div>
